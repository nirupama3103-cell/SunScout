...
const MapArea = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchPlaces = () => {
      ...
      const request = { query: searchTerm, location: new window.google.maps.LatLng(37.3541, -121.9552), radius: '5000' };
          ...
          activities.map(act => (
            <div key={act.id} className={styles.adventureCard}>
              ...
              <div className={styles.cardBody}>
                <p>📍 {act.address}</p>
                <button className={styles.actionBtn} onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(act.name + " " + act.address)}`, '_blank')}>
                  Go 🚗
                </button>
              </div>
            </div>
